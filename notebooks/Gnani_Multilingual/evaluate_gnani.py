"""
Gnani Prisma v2.5 ASR Evaluation Pipeline
Evaluates Gnani's Vachana STT API on Kathbath dataset.
Outputs: per-sample CSV, summary JSON, and an accumulating text report.
"""
...
MODEL_NAME = "gnani-prisma-v2.5"
FORMAT     = "transcribe"          # enables ITN; falls back to verbatim for non hi/en langs
...
# Be conservative on free tier; raise if you have higher rate limits
REQUEST_DELAY_SEC = 1.1

# ─── METRICS ─────────────────────────────────────────────────────────────────

def normalize(text: str) -> str:
    """Lowercase + collapse whitespace. Unicode-safe (works for Indic scripts)."""
...
def transcribe(audio_path: Path, language_code: str, api_key: str) -> Optional[str]:
    """
    Send one audio file to Gnani's Vachana STT REST API.
...
def evaluate_language(
    ...
    print(f"  Model    : {MODEL_NAME} / format={FORMAT}")
    ...
    for i, entry in enumerate(entries):
        ...
        # Resolve audio path (manifest paths are relative to dataset_root parent,
        # e.g. "kathbath/hindi/wavs/file.wav")
...
# ─── REPORT (accumulates across languages/runs) ────────────────────────────────
...
def write_report(summaries: list[dict], output_dir: Path):
    ...
    header = [
        "=" * 60,
        "  Gnani Prisma v2.5 ASR Evaluation Report",
        f"  Model     : {MODEL_NAME}  (format={FORMAT})",
        "=" * 60,
        "",
        ...
        with open(report_path) as f:
            for line in f:
                ...
                if stripped and not stripped.startswith("=") and not stripped.startswith("-") \
                        and not stripped.startswith("Gnani") and not stripped.startswith("Model") \
...
def main():
    parser = argparse.ArgumentParser(description="Gnani Prisma v2.5 ASR Evaluation Pipeline")
    parser.add_argument(
    ...
    parser.add_argument(
        "--output_dir", default="./gnani_results",
        help="Directory for results (default: ./gnani_results)"
    )
    parser.add_argument(
    ...
    if not args.api_key:
        raise ValueError(
            "No API key found. Pass --api_key or set GNANI_API_KEY environment variable."
        )
